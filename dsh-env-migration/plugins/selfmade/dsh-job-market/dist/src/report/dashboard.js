...
/** 组装市场总览（需求 §十五【市场总览】）。 */
export const buildOverview = (snapshot, config) => {
...
export const buildMySkillRows = (gap) => {
    if (gap === undefined)
        return [];
    return gap.entries.map((entry) => ({
...
/** 统计学习路线进度（需求 §十五【学习路线】）。 */
export const buildRoadmapProgress = (roadmap, goals, currentWeek = 1) => {
    ...
    const weeks = roadmap.weeks.map((week) => {
        const weekGoals = week.goal_ids.map((id) => goalById.get(id)).filter((goal) => goal !== undefined);
...
export const buildDashboardData = (input) => {
    ...
    const trends = buildAllTrends(history, 15);
    return {
...
export const traceSkill = (snapshot, jobs, skillId) => {
    const frequency = snapshot.skill_frequencies.find((item) => item.skill_id === skillId);
...
/** 回溯某个岗位方向的真实岗位。 */
export const traceCategory = (snapshot, jobs, category) => {
 ...
 * Markdown 报告
 * ==========================================================================*/
const pct = (value) => `${roundTo(value * 100, 1)}%`;
...
export const renderMarkdownReport = (input) => {
    ...
    skills.slice(0, 30).forEach((skill, index) => {
        const trend = skill.trend_pp === undefined
            ? '—'
            : `${skill.trend_pp > 0 ? '+' : ''}${roundTo(skill.trend_pp, 1)}pp`;
        lines.push(`| ${index + 1} | ${skill.skill} | ${skill.category} | ${skill.job_count} | ${pct(skill.job_ratio)} | ` +
    ...
    if (input.gap !== undefined && input.gap.entries.length > 0) {
        ...
        for (const entry of input.gap.entries.slice(0, 20)) {
            lines.push(`| ${entry.rank} | ${entry.skill} | ${pct(entry.market_demand)} | ${entry.current_level} | ${entry.target_level} | ` +
    ...
    if (input.roadmap !== null && input.roadmap !== undefined) {
        ...
        for (const week of input.roadmap.weeks) {
            ...
            for (const task of week.tasks)
                lines.push(`  - ${task}`);
            lines.push('- **验收条件**：');
            for (const criteria of week.acceptance)
                lines.push(`  - [ ] ${criteria}`);
        ...
        if (input.roadmap.project !== undefined) {
            ...
            lines.push('');
            lines.push(`完成该项目预计覆盖目标岗位 TOP${project.top20_total} 技能中的 **${project.top20_covered}** 项，` +
    ...
    if (dashboard.trends.length > 0) {
        ...
        for (const trend of dashboard.trends.slice(0, 15)) {
            ...
            lines.push(`| ${trend.skill} | ${pct(first.job_ratio)} | ${pct(last.job_ratio)} | ${trend.delta_pp > 0 ? '+' : ''}${roundTo(trend.delta_pp, 1)}pp | ${arrow} |`);
        }
        lines.push('');
    }
    lines.push('---');
...
/** 便捷：从历史中取最新快照。 */
export const currentSnapshot = (history) => latestSnapshot(history);
