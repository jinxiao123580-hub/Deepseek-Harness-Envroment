...
export class WorkspaceNotFoundError extends Error {
    ...
    cwd;
    constructor(details) {
        super(`无法解析当前会话的 Workspace（sessionId=${details.sessionId ?? '未提供'}，cwd=${details.cwd ?? '未提供'}）。` +
...
const resolveStatus = async (record) => {
...
export const resolveActiveWorkspace = async (ctx) => {
    ...
    if (sessionId !== undefined && sessionId !== '') {
        ...
        if (record !== undefined)
            return await toContext(record);
    }
    if (cwd !== undefined && cwd !== '') {
        const byPath = await registry.resolveByPath(cwd);
        if (byPath !== undefined)
            return await toContext(byPath);
        const byPrefix = registry.list().find((item) => resolve(item.path) === resolve(cwd));
        if (byPrefix !== undefined)
            return await toContext(byPrefix);
    }
    const all = registry.list();
    if (all.length === 1 && all[0] !== undefined)
        return await toContext(all[0]);
    throw new WorkspaceNotFoundError({ sessionId, cwd });
};
/** 相对工作区的绝对路径。 */
export const resolveOutputRoot = (workspace, relativePath) => join(workspace.path, relativePath);
...
export const writeFileAtomic = async (path, content) => {
...
export const readJsonFile = async (path) => {
    ...
    try {
        ...
        throw new Error(`JSON 文件内容损坏：${path}（${error instanceof Error ? error.message : String(error)}）`);
    }
};
export const writeJsonFile = async (path, value) => {
    await writeFileAtomic(path, `${JSON.stringify(value, null, 2)}\n`);
};
export const readWorkspaceJson = async (root, relativePath) => readJsonFile(join(root, relativePath));
export const writeWorkspaceJson = async (root, relativePath, value) => writeJsonFile(join(root, relativePath), value);
