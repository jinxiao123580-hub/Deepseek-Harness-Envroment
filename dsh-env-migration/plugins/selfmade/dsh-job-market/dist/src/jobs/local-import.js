 ...
 */
import { normalizeEducation, normalizeWhitespace, parseExperience, parseLocation, parseSalary, } from '../shared/text.js';
export const detectFormat = (path) => {
...
const KNOWN_SOURCES = ['boss', 'liepin', 'zhaopin', '51job', 'iguopin', 'local', 'manual'];
const toSource = (value) => {
    ...
    return aliases[normalized] ?? (KNOWN_SOURCES.includes(normalized) ? normalized : 'local');
};
/** 中英文列名 → 标准字段。 */
...
const assemble = (raw, fallbackSource, index) => {
    ...
    if (location.raw !== '')
        input.location = location.raw;
    if (location.city !== undefined)
        input.city = location.city;
    if (location.district !== undefined)
        input.district = location.district;
    // 只透传原文：数值统一由 JobStore.createJob 里的 parseSalary / parseExperience 解析，
    // 保证「导入」与「采集」两条入口的薪资/经验口径完全一致。
    if (salary.text !== '')
        input.salary_text = salary.text;
    if (experience.text !== '')
        input.experience_text = experience.text;
    const education = normalizeEducation(raw.education ?? '');
    if (education !== undefined)
        input.education = education;
    if (raw.industry !== undefined && raw.industry !== '')
        input.industry = normalizeWhitespace(raw.industry);
...
/** 解析 JSON 内容。 */
export const parseJsonJobs = (content, fallbackSource = 'local') => {
    const parsed = JSON.parse(content);
    const list = Array.isArray(parsed)
        ? parsed
        : typeof parsed === 'object' && parsed !== null && Array.isArray(parsed.jobs)
            ? (parsed.jobs)
            : [parsed];
    const results = [];
    list.forEach((item, index) => {
        ...
        // 数值只在一个地方解析，避免两条口径漂移。
        const numericText = (minKey, maxKey, unit) => {
...
/** 解析 CSV 内容。 */
export const parseCsvJobs = (content, fallbackSource = 'local') => {
...
export const parseMarkdownJobs = (content, fallbackSource = 'local') => {
    const results = [];
    ...
    blocks.forEach((block, index) => {
        ...
        if (heading === null || heading === undefined)
            return;
        const raw = { job_title: normalizeWhitespace(heading[1] ?? '') };
        const bodyLines = [];
...
/** 按格式解析岗位文本。 */
export const parseJobContent = (content, format, fallbackSource = 'local') => {
    if (format === 'csv')
        return parseCsvJobs(content, fallbackSource);
    if (format === 'markdown')
        return parseMarkdownJobs(content, fallbackSource);
