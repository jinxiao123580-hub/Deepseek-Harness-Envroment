...
const takenAtKey = (snapshot) => typeof snapshot.taken_at === 'string' ? snapshot.taken_at : '';
/** 按 `taken_at` 升序（稳定排序，相同时间保持原顺序）。 */
const byTakenAtAsc = (a, b) => {
    const left = takenAtKey(a);
    const right = takenAtKey(b);
    return left < right ? -1 : left > right ? 1 : 0;
};
/** 在快照里查找某技能的频次记录。 */
const findFrequency = (snapshot, skillId) => {
...
/** 0..1 比例 → `68.0%`。 */
const formatPercent = (ratio) => `${roundTo(ratio * 100, 1).toFixed(1)}%`;
/** 百分点差值 → `+7.0` / `-7.0`。 */
const formatDeltaPp = (deltaPp) => `${deltaPp >= 0 ? '+' : ''}${roundTo(deltaPp, 1).toFixed(1)}`;
/* ============================================================================
 * 一、历史维护
 * ==========================================================================*/
/**
 ...
 */
export const appendSnapshot = (history, snapshot, maxEntries = DEFAULT_MAX_SNAPSHOTS) => {
    const limit = isFiniteNumber(maxEntries) && maxEntries > 0 ? Math.floor(maxEntries) : DEFAULT_MAX_SNAPSHOTS;
    ...
    for (const entry of previous) {
        if (!isUsableSnapshot(entry))
            continue;
        if (!byId.has(entry.snapshot_id))
            order.push(entry.snapshot_id);
        byId.set(entry.snapshot_id, entry);
    }
    if (isUsableSnapshot(snapshot)) {
        if (!byId.has(snapshot.snapshot_id))
            order.push(snapshot.snapshot_id);
        // 同 ID 覆盖：以本次写入的快照为准。
    ...
    const updatedAt = newest !== undefined && takenAtKey(newest) !== ''
        ? takenAtKey(newest)
...
const chronologicalSnapshots = (history) => {
...
export const latestSnapshot = (history) => {
 ...
 */
export const buildSkillTrends = (history, skillIds) => {
    const snapshots = chronologicalSnapshots(history);
    ...
    for (const skillId of uniquePreserveOrder(requested)) {
        ...
        for (const snapshot of snapshots) {
            ...
            if (frequency === undefined)
                continue;
            if (typeof frequency.skill === 'string' && frequency.skill !== '')
                skillName = frequency.skill;
...
const stripGoalPrefix = (value) => (value.startsWith('goal-') ? value.slice(5) : value);
/**
 * 还原上一份路线覆盖的技能。
 *
 ...
 */
const collectPreviousGoals = (roadmap) => {
...
export const diffRoadmap = (input) => {
    const generatedAt = typeof input.now === 'string' && input.now !== '' ? input.now : new Date().toISOString();
    ...
    const priorityOrder = uniquePreserveOrder((Array.isArray(input.newPriorityOrder) ? input.newPriorityOrder : []).filter((skillId) => typeof skillId === 'string' && skillId !== ''));
    ...
    for (const goal of previousGoals) {
        ...
        if (current === undefined) {
            ...
            if (isFiniteNumber(beforeRatio) && beforeRatio >= ROADMAP_REMOVE_MIN_PREVIOUS_RATIO) {
                changes.push({
                    ...
                    skill_id: skillId,
                    skill: goal.skill,
                    reason: `${goal.skill}（${skillId}）在最新快照中已没有任何岗位要求` +
        ...
        const skillName = typeof current.skill === 'string' && current.skill !== '' ? current.skill : goal.skill;
        const currentRatio = isFiniteNumber(current.job_ratio) ? current.job_ratio : 0;
        const deltaPp = before !== undefined && isFiniteNumber(before.job_ratio)
        ...
        if (deltaPp !== undefined && deltaPp >= ROADMAP_DELTA_THRESHOLD_PP && topPriority.includes(skillId)) {
            ...
            changes.push({
                ...
                skill_id: skillId,
                skill: skillName,
                reason: `${skillName} 岗位占比从 ${formatPercent(beforeRatio ?? 0)} 升至 ${formatPercent(currentRatio)}` +
        ...
        if (deltaPp !== undefined && deltaPp <= -ROADMAP_DELTA_THRESHOLD_PP) {
            ...
            changes.push({
                ...
                skill_id: skillId,
                skill: skillName,
                reason: `${skillName} 岗位占比从 ${formatPercent(beforeRatio ?? 0)} 降至 ${formatPercent(currentRatio)}` +
        ...
        changes.push({
            ...
            skill_id: skillId,
            skill: skillName,
            reason: `${skillName} 当前被 ${current.job_count ?? 0} 个岗位要求` +
    ...
    for (const skillId of topPriority) {
        if (coveredSkillIds.has(skillId))
            continue;
        const current = findFrequency(newSnapshot, skillId);
        const skillName = current !== undefined && typeof current.skill === 'string' && current.skill !== ''
            ? current.skill
            : skillId;
    ...
    const summary = `本次仅产出增量调整建议（policy=incremental），不会推翻你的原计划：` +
    ...
    return {
        ...
        previous_snapshot_id: previousSnapshot?.snapshot_id ?? previousRoadmap?.based_on_snapshot_id ?? '',
