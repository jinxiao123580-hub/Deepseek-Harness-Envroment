...
import { loadTaxonomy, buildSkillIndex, buildSkillNameMap, buildCategoryLabelMap, createSkillDictionary, defaultTaxonomyJson, } from '../taxonomy/taxonomy.js';
...
import { JD_CACHE_PATH, cacheStats, emptyJdCache, pruneJdCache, readCachedAnalysis, writeCachedAnalysis, } from '../jd/jd-cache.js';
...
import { ensureOutputTree, readWorkspaceJson, writeWorkspaceJson, } from '../workspace/workspace.js';
...
export const createWorkflowContextFromConfig = async (input) => createWorkflowContext({ workspaceRoot: input.workspaceRoot, config: parseConfig(input.configInput) });
/* ============================================================================
 * 读 / 写
 * ==========================================================================*/
export const readJobs = async (ctx) => (await readWorkspaceJson(ctx.workspaceRoot, PATHS.jobs)) ?? [];
export const writeJobs = async (ctx, jobs) => writeWorkspaceJson(ctx.workspaceRoot, PATHS.jobs, jobs);
export const readHistory = async (ctx) => (await readWorkspaceJson(ctx.workspaceRoot, PATHS.snapshots)) ?? emptyHistory();
export const readJdCache = async (ctx) => (await readWorkspaceJson(ctx.workspaceRoot, PATHS.jdCache)) ?? emptyJdCache();
export const writeJdCache = async (ctx, cache) => writeWorkspaceJson(ctx.workspaceRoot, PATHS.jdCache, cache);
export const readPersonalProfile = async (ctx) => migratePersonalProfile(await readWorkspaceJson(ctx.workspaceRoot, PATHS.personalProfile));
export const writePersonalProfile = async (ctx, profile) => writeWorkspaceJson(ctx.workspaceRoot, PATHS.personalProfile, profile);
export const readGoals = async (ctx) => (await readWorkspaceJson(ctx.workspaceRoot, PATHS.goals)) ?? [];
export const writeGoals = async (ctx, goals) => writeWorkspaceJson(ctx.workspaceRoot, PATHS.goals, goals);
export const readRoadmap = async (ctx) => readWorkspaceJson(ctx.workspaceRoot, PATHS.roadmap);
export const writeRoadmap = async (ctx, roadmap) => writeWorkspaceJson(ctx.workspaceRoot, PATHS.roadmap, roadmap);
/** 首次配置时把完整 taxonomy 落盘，供用户人工编辑。 */
export const ensureTaxonomyFile = async (ctx) => {
    const path = `${ctx.workspaceRoot}/${PATHS.taxonomy}`;
    const existing = await readWorkspaceJson(ctx.workspaceRoot, PATHS.taxonomy);
...
/** 写入求职方向配置。 */
export const writeJobSearch = async (ctx, search) => writeWorkspaceJson(ctx.workspaceRoot, PATHS.jobSearch, search);
export const readJobSearch = async (ctx) => readWorkspaceJson(ctx.workspaceRoot, PATHS.jobSearch);
/**
 * 为岗位池构建 JD 分析结果。
 *
 ...
 */
export const buildAnalyses = async (ctx, jobs, options = {}) => {
...
export const prepareJdPrompt = (ctx, job) => buildJdAnalysisPrompt({ job, promptVersion: ctx.config.llm.prompt_version });
/** 把模型返回的结构化 JD 回灌进缓存。 */
export const ingestJdAnalysis = async (ctx, input) => {
...
export const analyzeMarket = async (ctx, options = {}) => {
    ...
    const gap = Object.keys(personal.skills).length > 0
...
export const planLearning = async (ctx, options = {}) => {
    const warnings = [...ctx.warnings];
    ...
    if (confirmedCount === 0) {
        warnings.push('尚未录入任何已确认的个人技能等级：请先用 job_market_set_skill 录入 0-5 级能力与证据。' +
            '系统不会自行假定你已掌握某项技能。');
    }
    /*
     * 未评估技能的处理。
    ...
    if (autoAssessed.length > 0) {
        warnings.push(`有 ${autoAssessed.length} 个市场高频技能你尚未评估等级，本次按 0 级临时纳入计划：` +
    ...
    const template = options.projectId === undefined
        ? PROJECT_CATALOG[0]
        : PROJECT_CATALOG.find((item) => item.project_id === options.projectId) ?? PROJECT_CATALOG[0];
    const frequencies = new Map(snapshot.skill_frequencies.map((item) => [
        item.skill_id,
    ...
    const project = template === undefined ? undefined : instantiateProject(template, frequencies, snapshot.job_count, { topSkillIds });
...
export const updateMarket = async (ctx) => {
    const warnings = [...ctx.warnings];
...
export const getStatus = async (ctx) => {
    ...
    const status = {
        ...
        cityCount: new Set(jobs.map((job) => job.city).filter((city) => city !== undefined)).size,
    ...
    if (snapshot !== undefined)
        status.latestSnapshotId = snapshot.snapshot_id;
    if (roadmap !== undefined)
        status.roadmapMode = roadmap.mode;
...
/** 技能追溯（需求 §十六）。 */
export const traceSkillInWorkspace = async (ctx, skillQuery) => {
    const history = await readHistory(ctx);
    const snapshot = latestSnapshot(history);
    if (snapshot === undefined)
        return undefined;
    ...
    // 退化：按展示名模糊匹配
    const byName = snapshot.skill_frequencies.find((item) => item.skill.includes(skillQuery) || item.skill_id.includes(skillQuery));
    return byName === undefined ? undefined : traceSkill(snapshot, jobs, byName.skill_id);
};
/** 汇总趋势（需求 §十四）。 */
export const getTrends = async (ctx, limit = 20) => {
    const history = await readHistory(ctx);
    return buildAllTrends(history, limit);
};
/** 个人画像摘要。 */
export const getPersonalSummary = async (ctx) => {
    const profile = await readPersonalProfile(ctx);
    return summarizeProfile(profile);
};
/** 在导入/采集后规范化一批岗位输入并写入岗位池（去重 + last_seen_at 语义）。 */
export const ingestJobInputs = async (ctx, inputs) => {
    const existing = await readJobs(ctx);
    const now = ctx.now();
    const normalizeSkills = (text) => canonicalSkillIds(text, ctx.dictionary, ctx.skillIndex);
...
/** 岗位池指纹，便于判断是否需要重算。 */
export const jobsFingerprintOf = (jobs) => hashValue('jobs', jobs.map((job) => [job.job_id, job.last_seen_at, job.seen_count]));
/** 空个人画像（首次配置用）。 */
export const newPersonalProfile = (targetRoles) => emptyPersonalProfile({ target_roles: [...targetRoles] });
//# sourceMappingURL=market-workflow.js.map