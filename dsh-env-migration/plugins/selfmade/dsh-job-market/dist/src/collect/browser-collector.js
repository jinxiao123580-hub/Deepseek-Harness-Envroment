 ...
 */
const SAFE_BSK_SUBCOMMANDS = ['session', 'navigate', 'snapshot', 'status', '--version'];
 ...
 */
export const checkBrowserSkill = async (executable, runner) => {
    const activeRunner = runner ?? createSafeBskRunner(executable);
    let lastMessage;
    for (const probe of [['--version'], ['status']]) {
        try {
...
/** 校验命令结果，非 0 退出即抛错。 */
const assertCommandOk = (result, command) => {
    if (result.exitCode !== 0) {
...
export const collectWithBrowserSkill = async (request, runner) => {
    ...
    if (request.userApproved !== undefined)
        policyRequest.userApproved = request.userApproved;
    if (request.executable !== undefined)
        policyRequest.executable = request.executable;
    if (request.source !== undefined)
        policyRequest.source = request.source;
    if (request.collectedAt !== undefined)
        policyRequest.collectedAt = request.collectedAt;
...
/** 快照必须是 JSON 对象。 */
const readSnapshotPayload = (stdout) => {
    let parsed;
    try {
...
/** 读取辅助需求标记字段。 */
const readAssistanceField = (payload) => {
    if (!isRecord(payload))
        return undefined;
 ...
 */
export const isAbortOutcome = (outcome) => outcome.status === 'needs-human';
/** 便于调用方构造 `needs-human` 结果的辅助（保持 abort 语义一致）。 */
export const needsHumanOutcome = (platform, target, humanReason, reason) => {
    const outcome = {
        platform,
        target,
        status: 'needs-human',
...
//# sourceMappingURL=browser-collector.js.map