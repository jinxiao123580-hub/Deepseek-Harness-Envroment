...
const BASE_PATTERN = new RegExp(`(${[...BASE_MODIFIERS]
    .sort((a, b) => b.length - a.length)
    .map((base) => base.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))
    .join('|')})`);
/** 从用户给定的岗位名中抽取方向词。同一岗位名只取最长的第一个方向词。 */
const extractBases = (role) => {
 ...
 */
export const expandKeywords = (config) => {
    ...
    for (const base of promptBases) {
        ...
        for (const suffix of ROLE_SUFFIXES) {
            if (allowed !== undefined && !allowed.includes(suffix))
                continue;
 ...
 */
export const keywordFamilies = (keywords) => {
...
const resolveCityValue = (platform, city) => {
...
const buildUrl = (platform, keyword, city, page) => {
...
export const planCollection = (input) => {
    ...
    const configuredCities = (Array.isArray(config.locations) ? config.locations : [])
        ...
        .filter((city) => city !== '');
    const cities = configuredCities.length > 0 ? uniquePreserveOrder(configuredCities) : [undefined];
    ...
    outer: for (const platform of platforms) {
        for (const keyword of expandedKeywords) {
            if (keyword === '' || excluded.has(keyword))
                continue;
            for (let page = 1; page <= pages; page += 1) {
                for (const city of cities) {
                    ...
                    const target = { platform: platform.id, keyword, page, url };
                    if (city !== undefined)
                        target.city = city;
                    targets.push(target);
                }
...
//# sourceMappingURL=search-planner.js.map