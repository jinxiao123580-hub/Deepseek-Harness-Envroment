 ...
 */
/** 全角转半角（含全角空格 U+3000）。 */
export const toHalfWidth = (value) => value
...
export const normalizeLookupKey = (value) => normalizeText(value)
    .replace(/[\s\u00b7\-—_/\\|,，.。;；:：!！?？"'“”‘’`~^*+#&@$%()（）[\]【】{}<>《》]/g, '')
    .trim();
/** 保序去重。 */
export const uniquePreserveOrder = (values) => {
    const seen = new Set();
    const result = [];
...
export const urlHostname = (value) => {
    try {
...
const COMPANY_SUFFIX_PATTERN = /(股份有限公司|有限责任公司|有限公司|科技有限公司|网络科技|信息技术|企业管理|人力资源|分公司|公司|集团|科技|实业|商贸)$/g;
/**
 * 公司名归一化，用于跨平台去重。
 * 去掉分支机构括号（如「（上海）」）与常见公司后缀，使
 * 「XX科技有限公司」与「XX科技」判定为同一家。
 */
export const normalizeCompanyForDedupe = (value) => {
...
const TITLE_NOISE_PATTERN = /(高级|资深|初级|中级|专家|首席|急招|急聘|诚聘|招聘|双休|五险一金|包吃|包住|包住宿|提供住宿|周末双休|base|职位|岗位|全职|兼职|长期|短期|储备|管培生?)/g;
/**
 * 职位名归一化，用于跨平台去重。
 * 例如「高级机器人软件工程师（ROS2）」与「机器人软件工程师」→ 同一个键。
 */
export const normalizeJobTitleForDedupe = (value) => {
...
export const parseSalary = (value) => {
    ...
    const result = { text };
    if (min !== undefined)
        result.min = roundTo(min, 1);
    if (max !== undefined && result.min !== undefined && max > result.min)
        result.max = roundTo(max, 1);
    if (months !== undefined)
 ...
 */
export const parseExperience = (value) => {
    const text = normalizeWhitespace(toHalfWidth(value ?? ''));
    if (text === '')
        return { text };
    if (/(不限|无要求|无经验要求|经验不限)/.test(text))
        return { text };
    if (/(应届|在校|实习|无经验|毕业生)/.test(text))
        return { min: 0, max: 1, text };
