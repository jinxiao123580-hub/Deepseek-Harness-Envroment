...
export const DEFAULT_LOCATIONS = ['上海', '杭州', '苏州', '南京'];
export const defaultJobSearch = {
...
export const DEFAULT_BROWSER_POLICY_CONFIG = {
    enabled: true,
    executable: 'bsk',
    mode: 'read-only',
...
const asStringArray = (value, fallback) => Array.isArray(value)
    ...
    : [...fallback];
const asNumber = (value, fallback, field) => {
...
const asPositiveInt = (value, fallback, field) => {
...
const asRoadmapWeeks = (value, fallback) => {
    if (value === undefined || value === null)
        return fallback;
...
export const parseConfig = (input) => {
    ...
    const jobSearch = {
        ...
        max_pages_per_keyword: asPositiveInt(searchInput.max_pages_per_keyword, defaultJobSearch.max_pages_per_keyword, 'jobSearch.max_pages_per_keyword'),
    };
    ...
    if (experienceMin !== undefined)
        jobSearch.experience_min = experienceMin;
    if (experienceMax !== undefined)
        jobSearch.experience_max = experienceMax;
    if (salaryMinK !== undefined)
        jobSearch.salary_min_k = salaryMinK;
    ...
    const browserSkill = {
        ...
        executable: typeof browserInput.executable === 'string' && browserInput.executable.trim() !== ''
        ...
        maxItemsPerRun: asPositiveInt(browserInput.maxItemsPerRun, DEFAULT_BROWSER_POLICY_CONFIG.maxItemsPerRun, 'browserSkill.maxItemsPerRun'),
        minIntervalMs: asPositiveInt(browserInput.minIntervalMs, DEFAULT_BROWSER_POLICY_CONFIG.minIntervalMs, 'browserSkill.minIntervalMs'),
    };
    const llm = {
        ...
        max_calls_per_run: asPositiveInt(llmInput.max_calls_per_run, defaultLlmCost.max_calls_per_run, 'llm.max_calls_per_run'),
    };
    const market = {
        recent_days: asNumber(marketInput.recent_days, defaultMarket.recent_days, 'market.recent_days') ?? defaultMarket.recent_days,
        min_jobs_for_stats: asPositiveInt(marketInput.min_jobs_for_stats, defaultMarket.min_jobs_for_stats, 'market.min_jobs_for_stats'),
        min_cooccurrence_count: asPositiveInt(marketInput.min_cooccurrence_count, defaultMarket.min_cooccurrence_count, 'market.min_cooccurrence_count'),
        top_skills_per_category: asPositiveInt(marketInput.top_skills_per_category, defaultMarket.top_skills_per_category, 'market.top_skills_per_category'),
    };
    const gapWeights = {
        ...DEFAULT_GAP_WEIGHTS,
        ...weightsInput,
...
export const resolvedAllowedDomains = (config) => [
