 ...
 */
export const DEFAULT_GAP_WEIGHTS = {
    market_demand: 1,
    importance: 1.2,
    skill_gap: 1,
    role_coverage: 0.8,
 ...
 */
export const computePriority = (factors, weights) => {
    ...
    const numerator = safePow(marketDemand, wMarketDemand) *
...
export const learningCostFor = (category, weights, opts) => {
...
const categoryRatio = (category, skillId, snapshot, frequency, presence) => {
    ...
    const ratio = total > 0 ? Math.min(1, (frequency?.job_count ?? 0) / total) : clamp01(frequency?.weighted_demand ?? 0);
    return { ratio, total, appeared: (frequency?.job_count ?? 0) > 0 };
};
/** 目标方向列表：显式入参 > 画像目标方向 > 快照中岗位数最多的方向（保证确定性）。 */
const resolveTargetCategories = (explicit, profile, snapshot) => {
...
export const analyzeGap = (input) => {
    ...
    const now = typeof input.now === 'string' && input.now.trim() !== ''
        ? input.now
    ...
    for (const skill of confirmed) {
        ...
        const currentLevel = finiteOr(skill.current_level, 0);
        const targetLevel = finiteOr(skill.target_level, currentLevel);
        ...
        const coverageText = targets.length === 0 ? '未设置目标方向（按 1 计）' : `${coverageHits}/${targets.length} 个目标方向`;
        const explanation = `${displayName}：市场加权需求 ${round2(marketDemand)}（${percent(marketDemand)}% 岗位要求，` +
