...
export const DEFAULT_ROADMAP_WEEKS = 8;
export const DEFAULT_HOURS_PER_WEEK = 15;
/** 每个难度点折算的学习小时数。 */
export const HOURS_PER_DIFFICULTY = 4;
export const VALID_ROADMAP_MODES = [4, 8, 12];
/**
 * 学习依赖相位。数字越小越先学。
...
const phaseOf = (category) => category === undefined ? 6 : PHASE_RANK[category] ?? 6;
const isMode = (value) => value === 4 || value === 8 || value === 12;
/** 把有序目标尽量均匀地切成 weeks 份（前面的周可以多 1 个目标）。 */
const chunkEvenly = (items, weeks) => {
...
const buildWhy = (weekGoals, snapshot, categoryName) => {
    ...
    return (`${names} 在最近统计窗口内出现在 ${jobCount} 个真实岗位中（覆盖 ${pct(coverage)} 的目标岗位），` +
        `属于 ${categoryName} 方向的高频要求；先补齐这些能力可以直接解锁最多的岗位。`);
};
/** 本周项目产出：优先绑定项目阶段，否则给出通用可验收产出。 */
const buildDeliverable = (weekGoals, project, stageIndex) => {
 ...
 */
export const planRoadmap = (input) => {
    const now = input.now ?? new Date().toISOString();
    const mode = isMode(input.mode) ? input.mode : DEFAULT_ROADMAP_WEEKS;
    const hoursPerWeek = input.hoursPerWeek ?? DEFAULT_HOURS_PER_WEEK;
    // skill_id → category，用于相位排序
    const categoryBySkill = new Map(input.snapshot.skill_frequencies.map((item) => [item.skill_id, item.category]));
    const ordered = [...input.goals].sort((a, b) => {
        ...
        if (b.market_job_count !== a.market_job_count)
            return b.market_job_count - a.market_job_count;
    ...
    for (let index = 0; index < mode; index += 1) {
        ...
        const tasks = weekGoals.flatMap((goal) => goal.learning_content.length > 0
        ...
        const goalTitle = weekGoals.length === 0
 ...
 */
export const completeGoalInRoadmap = (goals, input) => {
...
//# sourceMappingURL=learning-roadmap.js.map