...
export const PROJECT_CATALOG = [
    {
        ...
        name: '移动机器人全栈项目',
        description: '从 MCU 底层一路做到 ROS2 自主导航：STM32/ESP32 采集电机、编码器、IMU → micro-ROS 接入 ROS2 → ' +
    ...
    },
    {
        ...
        name: '嵌入式运动控制项目',
        description: '用一颗 MCU 完成一台电机的闭环控制：FOC 电流环 → 速度环 → 位置环，PID 整定、CAN 通信与实时性保障，' +
    ...
    },
    {
        project_id: 'ros2-arm',
        name: 'ROS2 机械臂控制项目',
        description: '从 URDF 建模到 MoveIt2 规划执行：正逆运动学、轨迹规划、ros2_control 硬件接口与力控，' +
    ...
    },
    {
        ...
        description: '用激光雷达（可选 IMU）做建图、定位与自主导航：Cartographer/LIO-SAM 建图、回环与后端优化、' +
    ...
    },
    {
        ...
        name: '视觉感知流水线项目',
        description: '搭建一条真实的视觉流水线：相机标定 → 图像处理 → 特征/检测 → 点云处理 → 手眼标定，' +
    ...
    },
    {
        ...
        name: '强化学习与具身智能项目',
        description: '在 MuJoCo/Isaac 里训练一个连续控制策略：MDP 建模 → PPO/SAC 训练 → 域随机化 sim2real → ' +
...
const topSkillsByJobCount = (skillFrequencies, size) => [...skillFrequencies.entries()]
    .sort((a, b) => {
 ...
 */
export const instantiateProject = (template, skillFrequencies, totalJobs, opts) => {
    const stages = [];
    const coveredSkillIds = [];
    ...
    for (const stage of template.stages) {
        ...
        for (const skillId of declared) {
            ...
            if (frequency === undefined)
                continue;
            if (!coveredSkillIds.includes(skillId))
                coveredSkillIds.push(skillId);
            for (const jobId of frequency.job_ids ?? [])
    ...
    const explicitTop = opts?.topSkillIds !== undefined && opts.topSkillIds.length > 0
