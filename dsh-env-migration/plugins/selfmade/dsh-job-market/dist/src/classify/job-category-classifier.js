...
/** 分类 ID → 中文标签。 */
export const CATEGORY_LABELS = new Map(CATEGORY_RULES.map((rule) => [rule.category, rule.label]));
/* ============================================================================
 * 匹配原语
 * ==========================================================================*/
/** 归一化待匹配文本：半角化 → 小写 → 空白折叠。 */
const normalizeForMatch = (value) => normalizeWhitespace(toHalfWidth(value)).toLowerCase();
/** 纯 ASCII 信号：需要在替换时补词边界，避免 `ros` 命中 `ros2`。 */
const isAsciiSignal = (keyword) => /^[\x20-\x7e]+$/.test(keyword);
const escapeRegExp = (value) => value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
/** 缓存 `关键词 → 计数正则`，避免重复编译。 */
const countPatternCache = new Map();
const countPattern = (keyword) => {
...
const countOccurrences = (haystack, keyword) => {
    if (keyword === '' || haystack === '')
        return 0;
...
export const classifyJobs = (jobs) => {
    const result = new Map();
    for (const job of jobs) {
        if (job === null || typeof job !== 'object')
            continue;
        result.set(job.job_id, classifyJob({
            title: job.job_title ?? '',
...
const TRAILING_CITY_PATTERN = /[\s\-—·、|/]*[（(【\[]?(北京|上海|广州|深圳|杭州|南京|苏州|成都|武汉|西安|天津|重庆|长沙|郑州|青岛|合肥|宁波|东莞|佛山|无锡|厦门|福州|济南|大连|沈阳|哈尔滨|长春|昆明|南昌|贵阳|南宁|太原|石家庄|兰州|银川|西宁|乌鲁木齐|呼和浩特|海口|珠海|中山|惠州|温州|嘉兴|常州|南通|徐州|烟台|潍坊|泉州|绍兴|台州|金华|保定|洛阳)[市]?[）)】\]]?$/;
...
//# sourceMappingURL=job-category-classifier.js.map