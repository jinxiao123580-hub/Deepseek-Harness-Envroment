...
export const skillLevelLabel = (level) => SKILL_LEVEL_LABELS[level] ?? SKILL_LEVEL_LABELS[SKILL_LEVEL_MIN] ?? '完全不会';
/** 兼容旧数据：宽松读取运行期可能出现的非法等级（迁移用，不抛错）。 */
const safeLevelLabel = (level) => skillLevelLabel(coerceLevel(level));
/**
 * 严格校验等级：必须是 0..5 的整数，否则抛 `TypeError`（中文提示）。
 * 用于所有**用户输入**路径，保证用户不会意外写入脏数据。
 */
const assertSkillLevel = (value, field) => {
    const numeric = typeof value === 'number' ? value : Number.NaN;
    if (!Number.isInteger(numeric) || numeric < SKILL_LEVEL_MIN || numeric > SKILL_LEVEL_MAX) {
        throw new TypeError(`技能等级 ${field} 无效：${String(value)}。必须是 ${SKILL_LEVEL_MIN}..${SKILL_LEVEL_MAX} 的整数` +
...
const coerceLevel = (value) => {
    ...
    if (rounded < SKILL_LEVEL_MIN)
        return 0;
    if (rounded > SKILL_LEVEL_MAX)
        return 5;
    return rounded;
};
/** 归一化任意文本数组。 */
...
/** 取注入的 `now`，缺省用当前时间。 */
const resolveNow = (value) => typeof value === 'string' && value.trim() !== '' ? value : new Date().toISOString();
/** 浅拷贝 skill 记录，保留未知键（非破坏性）。 */
const cloneSkillRecord = (raw) => {
 ...
 * 未知键原样保留。
 */
const normalizeSkill = (value, key) => {
    const record = value;
 ...
 */
export const migratePersonalProfile = (input) => {
    ...
    return {
        ...
        pending_suggestions: Array.isArray(rawSuggestions)
            ...
            : [],
        target_categories: toStringArray(rawCategories),
 ...
 */
export const setSkillLevel = (profile, input) => {
    ...
    const currentLevel = input.current_level === undefined
    ...
    const evidence = input.evidence !== undefined ? input.evidence.map((item) => ({ ...item })) : (existing?.evidence ?? []);
...
/** 从已确认画像中移除某项技能（纯函数）。 */
export const removeSkill = (profile, skillId) => {
 ...
 */
export const suggestSkill = (profile, suggestion) => {
    ...
    const now = resolveNow(suggestion.now);
    const entry = {
        skill_id: skillId,
 ...
 */
export const confirmSuggestion = (profile, input) => {
    if (input === null || typeof input !== 'object' || input.confirmed !== true) {
        throw new TypeError('确认技能建议必须显式传入 confirmed: true：AI 建议不得自动生效，只有用户明确确认后才能写入个人能力画像。');
    }
    const skillId = typeof input.skill_id === 'string' ? input.skill_id.trim() : '';
    if (skillId === '') {
    ...
    if (pending === undefined) {
        throw new TypeError(`未找到技能 ${skillId} 的待确认建议：只有存在于 pending_suggestions 中的 AI 建议才能被确认。`);
    }
    // 用户可覆盖 AI 建议的等级。
    const currentLevel = input.current_level === undefined
        ? coerceLevel(pending.suggested_level)
        : assertSkillLevel(input.current_level, 'current_level');
    const targetLevel = input.target_level === undefined ? currentLevel : assertSkillLevel(input.target_level, 'target_level');
 ...
 */
export const confirmedSkills = (profile) => {
...
/** 生成给提示词 / 报告用的紧凑中文摘要。 */
export const summarizeProfile = (profile) => {
    ...
    const scope = categories.length === 0 && roles.length === 0
        ? '未设置'
        : [
    ...
    if (skills.length === 0) {
        ...
        const parts = skills.map((skill) => `${skill.skill_id} ${safeLevelLabel(skill.current_level)}（${coerceLevel(skill.current_level)}级）` +
...
//# sourceMappingURL=personal-skill-profile.js.map