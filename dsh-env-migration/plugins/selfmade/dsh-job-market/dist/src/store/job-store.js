...
import { normalizeCompanyForDedupe, normalizeEducation, normalizeJobTitleForDedupe, normalizeUrl, normalizeWhitespace, parseExperience, parseLocation, parseSalary, uniquePreserveOrder, } from '../shared/text.js';
...
export const buildPlatformKey = (input) => {
    ...
    return `fallback:${input.source}:${normalizeCompanyForDedupe(input.company)}|${normalizeJobTitleForDedupe(input.job_title)}|${normalizeWhitespace(input.city)}`;
};
/**
 * 跨平台、跨日期身份键。
 * 刻意 **不包含 source**，因此同一岗位在 BOSS 与猎聘上会被判定为同一岗位。
 */
export const buildDedupeKey = (input) => `${normalizeCompanyForDedupe(input.company)}|${normalizeJobTitleForDedupe(input.job_title)}|${normalizeWhitespace(input.city)}`;
/** 规范岗位 ID：`job-<fnv1a32(dedupeKey)>`（与上游同算法，便于互相核对）。 */
export const makeJobId = (dedupeKey) => `job-${fnv1a32(dedupeKey)}`;
/** 由采集/导入输入构造规范 `Job`。 */
export const createJob = (input, options = {}) => {
    ...
    const requirements = uniquePreserveOrder((input.requirements ?? []).map((item) => normalizeWhitespace(item)).filter((item) => item !== ''));
    ...
    if (salary.months !== undefined)
        job.salary_months = salary.months;
    if (salary.text !== '')
        job.salary_text = salary.text;
    if (experience.min !== undefined)
        job.experience_min = experience.min;
    if (experience.max !== undefined)
        job.experience_max = experience.max;
    if (experience.text !== '')
...
const extractRawSkillTokens = (text) => {
    const matches = text.match(/[A-Za-z][A-Za-z0-9+#.\-]{1,19}/g) ?? [];
    const chinese = text.match(/[\u4e00-\u9fa5]{2,8}(?:算法|控制|规划|框架|工具|协议|总线|系统|引擎|平台|模型)/g) ?? [];
    return uniquePreserveOrder([...matches, ...chinese].map((item) => item.trim()).filter((item) => item !== '')).slice(0, 120);
};
/** 合并同一岗位的两次观测。以 `existing` 为主体，只补充/刷新。 */
export const mergeJob = (existing, incoming, now) => {
    const sources = uniquePreserveOrder([...existing.sources, ...incoming.sources, incoming.source]);
    const merged = {
        ...existing,
        sources,
...
const fillNumberIfMissing = (target, key, value) => {
    if (target[key] === undefined && value !== undefined) {
        target[key] = value;
    }
};
const maxIso = (...values) => values.reduce((latest, current) => (current > latest ? current : latest), values[0] ?? '');
const minIso = (...values) => values.reduce((earliest, current) => (current < earliest ? current : earliest), values[0] ?? '');
/**
 * 增量合并岗位池。
 *
 ...
 */
export const upsertJobs = (existing, incoming, now) => {
    ...
    jobs.forEach((job, index) => {
        ...
        if (!byDedupe.has(job.dedupe_key))
            byDedupe.set(job.dedupe_key, index);
    });
    const newJobIds = [];
    const updatedJobIds = [];
    for (const candidate of incoming) {
        ...
        if (dedupeIndex === undefined) {
            ...
            if (!byDedupe.has(candidate.dedupe_key))
                byDedupe.set(candidate.dedupe_key, index);
...
/** 岗位池快速查询辅助。 */
export const indexJobsById = (jobs) => new Map(jobs.map((job) => [job.job_id, job]));
export const jobsByCategory = (jobs, category) => jobs.filter((job) => job.job_category === category);
export const jobsByCity = (jobs, city) => jobs.filter((job) => normalizeWhitespace(job.city) === normalizeWhitespace(city));
/** 为便于追溯与审计，生成岗位池指纹（内容变化即变化）。 */
export const jobsFingerprint = (jobs) => hashValue('jobs', jobs.map((job) => [job.job_id, job.last_seen_at, job.seen_count]));
