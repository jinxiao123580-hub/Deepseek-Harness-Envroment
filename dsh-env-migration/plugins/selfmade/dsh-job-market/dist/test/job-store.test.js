...
import { buildDedupeKey, buildPlatformKey, createJob, mergeJob, makeJobId, upsertJobs, } from '../src/store/job-store.js';
...
test('upsertJobs：跨平台同一岗位合并为一个岗位并记录两个来源', () => {
    ...
    const onLiepin = createJob(baseInput({
        source: 'liepin',
...
test('upsertJobs：跨日期增量只更新已存在岗位', () => {
    const first = createJob(baseInput({ collected_at: DAY1 }));
    const second = createJob(baseInput({
        job_title: 'SLAM算法工程师',
        company: '北京某公司',
...
test('mergeJob 补齐缺失字段但不覆盖已有值', () => {
    const sparse = createJob(baseInput({ url: 'https://www.zhipin.com/job_detail/a.html', salary_text: undefined, description: undefined }));
    const rich = createJob(baseInput({ url: 'https://www.zhipin.com/job_detail/b.html', salary_text: '30-50K' }));
