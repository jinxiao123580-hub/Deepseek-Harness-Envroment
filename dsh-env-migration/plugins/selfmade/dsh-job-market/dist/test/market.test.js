...
const normalizeSkills = (text) => canonicalSkillIds(text, dictionary, skillIndex);
/** 造一个岗位并顺手完成分类，等价于真实流水线里的分类步骤。 */
const makeJob = (input, now = NOW) => {
    ...
    return classified.category === 'unknown' ? created : { ...created, job_category: classified.category };
};
...
const buildAnalyses = (jobs) => {
    const map = new Map();
    for (const job of jobs) {
        map.set(job.job_id, parseJdByRule({
            job,
            dictionary,
            analysisVersion: 1,
            promptVersion: 1,
...
const snapshotOf = (jobs, analysesMap, now = NOW) => buildMarketSnapshot({
    jobs,
...
const freqOf = (id) => snapshot.skill_frequencies.find((item) => item.skill_id === id);
...
test('技能频率分别给出岗位数、出现率、Required 与 Preferred 比例', () => {
    ...
    assert.equal(ros2.required_count + ros2.preferred_count + ros2.bonus_count, ros2.job_count, '三档计数之和必须等于岗位数（每个岗位对每个技能只落一个强度档）');
...
test('每个百分比都可回溯到真实岗位 ID（需求 §十六）', () => {
    ...
    for (const id of ros2.job_ids)
        assert.ok(poolIds.has(id), `${id} 应在岗位池中`);
});
