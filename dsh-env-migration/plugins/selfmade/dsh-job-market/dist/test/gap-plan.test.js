...
const jobs = makeJobs();
const analyses = new Map(jobs.map((job) => [job.job_id, parseJdByRule({ job, dictionary, analysisVersion: 1, promptVersion: 1, now: NOW })]));
...
test('computePriority 严格等于公式：demand × importance × gap × coverage ÷ cost', () => {
    ...
    const expected = (Math.pow(0.7, DEFAULT_GAP_WEIGHTS.market_demand) *
...
test('ROS2 目标拆分出 Node/Topic/Service/Action/Parameter/Launch/TF2/URDF/QoS/rosbag/RViz/Nav2', () => {
    ...
    if (ros2Goal === undefined)
        return; // 该岗位池未把 ros2 排进前列时不强求
...
//# sourceMappingURL=gap-plan.test.js.map