...
export const scopedHash = (namespace: string, value: string): string =>
    `${namespace}-${fnv1a32(`${namespace}\u0000${value}`)}`;
...
/** 对任意 JSON 值求稳定哈希。 */
export const hashValue = (namespace: string, value: unknown): string =>
    scopedHash(namespace, stableStringify(value));

/** 取内容前 N 个字符 + 全量哈希，便于日志中显示可读标识。 */
export const shortHash = (value: string, prefixLength = 8): string =>
    `${value.slice(0, prefixLength)}…${fnv1a32(value)}`;
