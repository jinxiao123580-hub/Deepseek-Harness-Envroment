...
import type { GapReport, GapEntry, LearningGoal, MarketSnapshot, SkillFrequency, ProjectPlan } from '../shared/types.js';
...
const GENERIC_BREAKDOWN: readonly string[] = [
    '核心概念与术语',
    '最小可运行示例',
    '在项目中独立应用',
    '常见问题排查',
 ...
 * 这张表就是这条规则的可执行版本。
 */
export const CAPABILITY_BREAKDOWN: Record<string, string[]> = {
    /* ---- 需求 §十一 明确要求给出的能力点 ---- */
...
const breakdownFor = (skillId: string): string[] => {
...
const GENERIC_CRITERION = (sub: string): string =>
    `提交一个可运行的「${sub}」最小工程：代码入库 + 运行日志或截图，并书面说明关键参数与踩坑点`;
 ...
 */
const CONCRETE_CRITERIA: Record<string, Record<string, string>> = {
    ros2: {
...
const SKILL_FOCUS: Record<string, string> = {
...
/** 从能力点推标题里的能力描述（兜底路径）。 */
const focusFor = (skillId: string, subSkills: readonly string[]): string => {
    const registered = SKILL_FOCUS[skillId];
    if (registered !== undefined && registered !== '') return registered;
    const head = subSkills.slice(0, 3).join('、');
    return `${head} 等 ${subSkills.length} 项核心能力`;
};

/** 一条能力点 → 一条可观察的验收标准。 */
const criterionFor = (skillId: string, sub: string): string => {
...
const acceptanceFor = (skillId: string, subSkills: readonly string[]): string[] => {
...
const difficultyFor = (entry: GapEntry): number => clamp(Math.round(finiteOr(entry.learning_cost, 3)), 1, 5);

/** 覆盖率：优先用快照的 job_ratio，缺失时用 job_count / 总岗位数。 */
const coverageOf = (frequency: SkillFrequency, totalJobs: number): number => {
    const ratio = finiteOr(frequency.job_ratio, Number.NaN);
    if (Number.isFinite(ratio)) return clamp(ratio, 0, 1);
...
const compareEntries = (a: GapEntry, b: GapEntry): number => {
...
export const planGoals = (input: {
    ...
    for (const frequency of snapshot.skill_frequencies ?? []) {
        if (frequency === undefined || frequency === null) continue;
    ...
    const projectSkillIds = new Set<string>(
        input.project !== undefined && Array.isArray(input.project.covered_skill_ids)
    ...
    for (const entry of entries) {
        ...
        const skillName =
            typeof frequency.skill === 'string' && frequency.skill.trim() !== ''
