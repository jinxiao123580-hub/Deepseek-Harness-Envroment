...
export const normalizeAlias = (value: string): string => normalizeText(value);
 ...
 */
export const findAliasMatches = (
    text: string,
    dictionary: SkillDictionary,
): { alias: string; entry: SkillDictionaryEntry; index: number }[] => {
    const haystack = normalizeAlias(text);
    if (haystack === '') return [];

    // 收集所有可能的命中位置，然后按「起点升序、长度降序」贪心选取不重叠者。
    const candidates: { alias: string; entry: SkillDictionaryEntry; index: number }[] = [];
...
export const expandImplied = (skillId: string, index: SkillIndex): string[] => {
    ...
    while (queue.length > 0) {
        ...
        for (const implied of definition.implies ?? []) {
            if (visited.has(implied)) continue;
            visited.add(implied);
 ...
 */
export const normalizeSkillToken = (
    raw: string,
    dictionary: SkillDictionary,
    index: SkillIndex,
    ...
    const best = matches.reduce((longest, current) =>
        current.alias.length > longest.alias.length ? current : longest,
    );
    ...
    return {
        ...
        category: best.entry.category as SkillCategory,
 ...
 */
export const normalizeSkillTokens = (
 ...
 */
export const canonicalSkillIds = (text: string, dictionary: SkillDictionary, index: SkillIndex): string[] => {
...
export const primarySkillIds = (
    ...
    for (const match of findAliasMatches(text, dictionary)) {
        if (seen.has(match.entry.skill_id)) continue;
...
/** 批量归一化 + 未识别项报告。 */
export const normalizeWithReport = (
    raws: readonly string[],
    dictionary: SkillDictionary,
    index: SkillIndex,
): { normalized: NormalizedSkill[]; missed: NormalizationMiss[] } => {
    const normalized: NormalizedSkill[] = [];
    const missed: NormalizationMiss[] = [];
    for (const raw of raws) {
