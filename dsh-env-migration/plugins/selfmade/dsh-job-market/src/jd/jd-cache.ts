...
export const emptyJdCache = (): JdCacheFile => ({
 ...
 */
export const computeJobHash = (job: Job): string => hashValue('jd-job', [job.job_id, job.company, job.job_title]);

/** 组装用于解析的规范 JD 文本（岗位标题 + 描述 + 要求）。 */
export const jdTextOf = (job: Job): string =>
    normalizeText([job.job_title, job.description ?? '', ...job.requirements].filter((part) => part !== '').join('\n'));

/** 归一化 JD 文本的哈希。JD 文本未变则哈希不变 → 命中缓存。 */
export const computeJdHash = (job: Job): string => hashValue('jd-text', jdTextOf(job));
...
}): string => `${input.jobHash}:${input.jdHash}:a${input.analysisVersion}:p${input.promptVersion}`;

/** 从缓存读取仍有效的分析结果。版本不匹配或 JD 变更时返回 undefined（= 需要重算）。 */
export const readCachedAnalysis = (
    cache: JdCacheFile,
    input: { job: Job; analysisVersion: number; promptVersion: number },
): JdAnalysis | undefined => {
    if (cache.version !== JD_CACHE_SCHEMA_VERSION) return undefined;
    ...
    if (entry.jd_hash !== jdHash) return undefined;
    if (entry.analysis_version !== input.analysisVersion) return undefined;
    if (entry.prompt_version !== input.promptVersion) return undefined;
    return entry.analysis;
};

/** 把一次分析结果写回缓存（不可变：返回新对象）。 */
export const writeCachedAnalysis = (
    cache: JdCacheFile,
    input: { job: Job; analysis: JdAnalysis; now: Iso },
): JdCacheFile => {
    const jobHash = computeJobHash(input.job);
    const jdHash = computeJdHash(input.job);
    const key = cacheKey({
 ...
 */
export const pruneJdCache = (
    cache: JdCacheFile,
    input: { analysisVersion: number; promptVersion: number; maxEntries?: number },
): JdCacheFile => {
    const maxEntries = input.maxEntries ?? 20_000;
    const kept: [string, JdCacheEntry][] = [];
    for (const [key, entry] of Object.entries(cache.entries)) {
        if (entry.analysis_version !== input.analysisVersion) continue;
        if (entry.prompt_version !== input.promptVersion) continue;
        kept.push([key, entry]);
    }
    if (kept.length <= maxEntries) {
        const entries: Record<string, JdCacheEntry> = {};
        for (const [key, entry] of kept) entries[key] = entry;
    ...
    const entries: Record<string, JdCacheEntry> = {};
    for (const [key, entry] of kept.slice(0, maxEntries)) entries[key] = entry;
    return { version: JD_CACHE_SCHEMA_VERSION, entries, updated_at: cache.updated_at };
};

/** 缓存统计（供 status 工具做成本可见性）。 */
export const cacheStats = (cache: JdCacheFile): JdCacheStats => {
...
/** 按版本统计（更细的审计视图）。 */
export const cacheStatsByVersion = (
    cache: JdCacheFile,
    input: { analysisVersion: number; promptVersion: number },
): JdCacheStats => {
    const all = Object.values(cache.entries);
    const current = all.filter(
        (entry) => entry.analysis_version === input.analysisVersion && entry.prompt_version === input.promptVersion,
    );
    return {
        total: all.length,
