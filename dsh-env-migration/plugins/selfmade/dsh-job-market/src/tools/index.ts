...
import type { JobMarketConfig, JobSearchConfig, JobSource, PersonalSkillProfile, SkillEvidence, SkillLevel } from '../shared/types.js';
...
import {
    ...
    BrowserSkillUnavailableError,
    checkBrowserSkill,
    collectWithBrowserSkill,
    createSafeBskRunner,
} from '../collect/browser-collector.js';
...
import {
    analyzeMarket,
    createWorkflowContextFromConfig,
    ensureTaxonomyFile,
    getPersonalSummary,
    getStatus,
    getTrends,
    ...
    writeGoals,
    writeJobSearch,
    writePersonalProfile,
    PATHS,
} from '../workflow/market-workflow.js';
import type { WorkflowContext } from '../workflow/market-workflow.js';
...
/** JSON 安全值：与 dsh-tools 的 JsonValue 结构一致，保证 execute 返回值可序列化。 */
...
const SKILL_LEVELS: readonly number[] = [0, 1, 2, 3, 4, 5];

/** 把用户写的技能名（可能是别名）解析成稳定的 skill_id。 */
const resolveSkillRef = (ctx: WorkflowContext, raw: string): { skill_id: string; skill: string } => {
    const query = raw.trim();
    if (query === '') throw new TypeError('技能不能为空。');
 ...
 */
export const createJobMarketTools = (
    resolveWorkspace: ResolveWorkspace,
    config: JobMarketConfig,
): unknown[] => {
    // WorkflowContext 载入 taxonomy（约 180 个技能），按工作区缓存避免每次调用重读。
    const contextCache = new Map<string, Promise<WorkflowContext>>();
    const getContext = async (workspace: WorkspaceContext): Promise<WorkflowContext> => {
    ...
    return [
        ...
        defineTool({
            ...
            description:
                'Show the active Workspace, job-market state (job pool, snapshots, personal skill profile, roadmap) and configured safety defaults. [求职市场状态]',
            parameters: {},
            output: jsonOutput,
        ...
        defineTool({
            ...
            description:
                'Set the job-search target direction: roles, cities, experience and salary floor, extra keywords and exclusions. Persists to config/job-search.json in the active Workspace. [配置求职方向]',
            parameters: {
                target_roles: {
                    type: 'array',
                    items: { type: 'string' },
            ...
            async execute(args, exec) {
                const workspace = await resolveWorkspace(exec);
                const current = (await readJobSearch(await getContext(workspace))) ?? ({} as JobSearchConfig);
                const next: JobSearchConfig = {
                    ...current,
        ...
        defineTool({
            ...
            description:
                'Expand the configured target roles into concrete search keywords and build a read-only collection plan with allowlisted search URLs per platform. Call this BEFORE collecting, then show the URLs to the user. [生成采集计划]',
            parameters: {
            ...
            async execute(args, exec) {
                ...
                const browserStatus = await checkBrowserSkill(config.browserSkill.executable, createSafeBskRunner(
                    config.browserSkill.executable,
                ));
                return asJson({
                    plan,
                    platforms: PLATFORMS,
        ...
        defineTool({
            ...
            description:
                'Read visible job postings through the read-only Tencent/BrowserSkill session for allowlisted URLs, then merge and deduplicate them into the active Workspace job pool. Requires explicit user confirmation. [只读采集岗位]',
            parameters: {
                urls: {
                    type: 'array',
                    items: { type: 'string' },
            ...
            async execute(args, exec) {
                ...
                const merged = await ingestJobInputs(
                    ctx,
                    rawJobRecordsToInputs(raw, (args.source ?? 'browser-skill') as JobSource, ctx.now()),
                );
                return asJson({
                    source: args.source ?? 'browser-skill',
        ...
        defineTool({
            ...
            description:
                'Import job postings from a local JSON / CSV / Markdown file inside the active Workspace, then merge and deduplicate into the job pool. Use this when BrowserSkill is unavailable. [导入本地岗位]',
            parameters: {
            ...
            async execute(args, exec) {
                ...
                const inputs = parseJobContent(content, format, (args.source ?? 'local') as never);
                if (inputs.length === 0) {
                    throw new Error(
                        `未能从 ${args.path} 解析出任何岗位。请确认格式（${format}）与字段（至少需要岗位名称与公司名称）。`,
                    );
                }
        ...
        defineTool({
            ...
            description:
                'Classify every job, normalize skill mentions, parse JDs (rule-first with caching), then compute the market snapshot: skill frequency with required/preferred/bonus split, cooccurrence lift, per-direction breakdowns, salary and experience distributions. All numbers are computed by code, never by the model. [市场分析]',
            parameters: {
            ...
            async execute(args, exec) {
                ...
                const filter = {
                    ...
                    ...(args.categories === undefined ? {} : { categories: args.categories as never }),
                    ...(args.salary_min_k === undefined ? {} : { salary_min_k: args.salary_min_k }),
                    ...(args.experience_max === undefined ? {} : { experience_max: args.experience_max }),
                };
        ...
        defineTool({
            ...
            description:
                'Return the six-block dashboard (market overview, skill ranking, job directions, my capability, learning roadmap, trends) built from the latest snapshot. Includes provenance so every figure is traceable. [市场看板]',
            parameters: {},
            output: jsonOutput,
            async execute(_args, exec) {
                const workspace = await resolveWorkspace(exec);
        ...
        defineTool({
            ...
            description:
                'Prove a statistic: given a skill name, list the exact real jobs that require / prefer it, with company, title, city, url and source. Use this whenever the user challenges a number such as "ROS2 68%". [技能数据回溯]',
            parameters: {
            ...
            async execute(args, exec) {
                ...
                if (trace === undefined) {
                    ...
                    throw new Error(
                        `技能「${args.skill}」在当前快照里没有匹配记录（岗位池 ${jobs.length} 个）。` +
        ...
        defineTool({
            ...
            description:
                'Show how skill demand evolved across saved market snapshots (e.g. ROS2 61% → 68% → 72%), with direction and delta in percentage points. [需求趋势]',
            parameters: {
        ...
        defineTool({
            ...
            description:
                'Read the personal skill profile: 0-5 levels, target levels, evidence, last update and confidence, plus any pending AI suggestions that still need confirmation. [查看我的能力]',
            parameters: {},
            output: jsonOutput,
            async execute(_args, exec) {
                const workspace = await resolveWorkspace(exec);
        ...
        defineTool({
            ...
            description:
                'Record the user\'s own skill level (0=none, 1=concept, 2=demo, 3=independent project, 4=solves real problems, 5=deep mastery) with evidence, and the target level. Only the user may declare mastery; never infer it. [设置技能等级]',
            parameters: {
            ...
            async execute(args, exec) {
                ...
                const next = setSkillLevel(profile, {
                    ...
                    current_level: args.current_level as SkillLevel,
                    ...(args.target_level === undefined ? {} : { target_level: args.target_level as SkillLevel }),
                    ...(args.evidence === undefined ? {} : { evidence: args.evidence as unknown as SkillEvidence[] }),
        ...
        defineTool({
            ...
            description:
                'Confirm or dismiss an AI-suggested skill level. AI suggestions never take effect automatically: only an explicit confirmation writes them into the profile. [确认技能建议]',
            parameters: {
            ...
            async execute(args, exec) {
                ...
                if (args.confirmed !== true) {
                    ...
                    const dismissed: PersonalSkillProfile = {
                        ...profile,
                ...
                const next = confirmSuggestion(profile, {
                    skill_id: ref.skill_id,
                    confirmed: true,
                    ...(args.current_level === undefined ? {} : { current_level: args.current_level as SkillLevel }),
                    ...(args.target_level === undefined ? {} : { target_level: args.target_level as SkillLevel }),
                    now: ctx.now(),
                });
                await writePersonalProfile(ctx, next);
        ...
        defineTool({
            ...
            description:
                'Compute the skill gap (priority = market_demand × importance × skill_gap × role_coverage ÷ learning_cost, with per-skill explanation), then generate verifiable stage goals and a 4/8/12-week learning roadmap driven by a real robotics project. [生成学习计划]',
            parameters: {
            ...
            async execute(args, exec) {
                ...
                const result = await planLearning(ctx, {
                    ...(args.weeks === undefined ? {} : { weeks: args.weeks as 4 | 8 | 12 }),
                ...
                const markdown = renderMarkdownReport({
                    dashboard: (
                        await analyzeMarket(ctx, { persist: false, writeReport: false })
                    ).dashboard,
                    snapshot: result.snapshot,
                    gap: result.gap,
                    goals: result.goals,
        ...
        defineTool({
            ...
            description:
                'Mark a learning goal as done. Only allowed when the goal has explicit acceptance criteria and the user confirms they are all met. [标记目标完成]',
            parameters: {
        ...
        defineTool({
            ...
            description:
                'Re-analyze the market on the current job pool, append a new snapshot, and produce an incremental adjustment proposal (continue / add / raise / lower / remove) with reasons. It never overthrows the existing plan. [更新求职市场]',
            parameters: {},
            output: jsonOutput,
            async execute(_args, exec) {
                const workspace = await resolveWorkspace(exec);
        ...
        defineTool({
            ...
            description:
                'Return the structured-JD extraction prompt for one job. Use only when rule-based parsing is clearly insufficient (ambiguous seniority, mixed responsibilities). The model then returns ParsedJd JSON which you feed back via job_market_ingest_jd_analysis. [准备 JD 解析 prompt]',
            parameters: {
            ...
            async execute(args, exec) {
                ...
                if (job === undefined) throw new Error(`找不到岗位 ${args.job_id}（岗位池共 ${jobs.length} 个）。`);
        ...
        defineTool({
            ...
            description:
                'Write a model-produced ParsedJd back into the JD cache, normalizing its skills into taxonomy ids and required/preferred/bonus weights. The cache is keyed by JD hash + analysis/prompt version, so unchanged JDs are never re-analyzed. [回灌 JD 解析结果]',
            parameters: {
            ...
            async execute(args, exec) {
                ...
                const jobs = await readJobs(ctx);
                const job = jobs.find((item) => item.job_id === args.job_id);
                if (job === undefined) throw new Error(`找不到岗位 ${args.job_id}。`);
                const analysis = await ingestJdAnalysis(ctx, { job, parsed: args.parsed as never });
                return asJson({
                    job_id: job.job_id,
                    via: analysis.via,
        ...
        defineTool({
            ...
            description:
                'One-time setup for the active Workspace: create the output directory tree, write the editable skill taxonomy file, and initialize the personal skill profile from the configured target roles. Safe to re-run. [初始化工作区]',
            parameters: {},
            output: jsonOutput,
            async execute(_args, exec) {
                const workspace = await resolveWorkspace(exec);
