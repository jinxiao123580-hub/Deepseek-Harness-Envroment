...
export const JOB_CATEGORY_LABELS: Readonly<Record<string, string>> = {
...
const medianOfSorted = (sorted: readonly number[]): number => {
    ...
    if (length % 2 === 1) return sorted[middle] ?? Number.NaN;
...
const percentileOfSorted = (sorted: readonly number[], percentile: number): number => {
    const length = sorted.length;
    if (length === 0) return Number.NaN;
...
/** 收集某个岗位集合里出现的薪资下限（升序）。 */
const collectSortedSalary = (jobs: readonly Job[], key: 'salary_min' | 'salary_max'): number[] =>
    jobs
        .map((job) => job[key])
...
/** 从 JD 解析结果里观察到的技能标签，作为 `skillNames` 缺失时的兜底。 */
interface SkillLabelIndex {
    names: Map<string, string>;
    categories: Map<string, SkillCategory>;
}

const collectSkillLabelIndex = (
    jobs: readonly Job[],
    ...
    for (const job of jobs) {
        ...
        for (const mention of mentions) {
            if (mention === undefined || mention === null) continue;
            ...
            for (const impliedId of implied) {
                if (typeof impliedId !== 'string' || impliedId === '') continue;
...
const resolveSkillLabel = (
    skillId: string,
    index: SkillLabelIndex,
    skillNames?: ReadonlyMap<string, { name: string; category: string }>,
): SkillLabel => {
    ...
    if (provided !== undefined) {
        return {
            ...
            category:
                typeof provided.category === 'string' && provided.category !== ''
                    ? (provided.category as SkillCategory)
                    : 'unknown',
        };
    }
...
/**
 * 一个岗位的技能视图：
 *  - `present`：该岗位「出现」的全部技能（`skills_normalized` ∪ JD mentions ∪ implied）。
 *  - `required` / `preferred` / `bonus`：三档要求强度集合，每档每个技能最多计一次。
 *
 * 规则（需求 §五 / §七）：
 *  - 有 JD 解析：要求强度完全以 `mentions` 为准，`implied_skill_ids` 继承同样的强度
 *    （`C++17` 因此同时给 `cpp` 与 `modern-cpp` 记分）。
 *  - 无 JD 解析：退化策略——把 `skills_normalized` 当作硬性要求，避免漏计。
 */
...
const STRENGTH_RANK = { required: 3, preferred: 2, bonus: 1 } as const;
type Strength = keyof typeof STRENGTH_RANK;
 ...
 */
const buildJobSkillView = (job: Job, analysis: JdAnalysis | undefined): JobSkillView => {
    const present = new Set<string>();
    const strongest = new Map<string, Strength>();

    const record = (id: string, strength: Strength): void => {
        present.add(id);
    ...
    if (analysis !== undefined && analysis !== null) {
        ...
        for (const mention of mentions) {
            if (mention === undefined || mention === null) continue;
            const implied = Array.isArray(mention.implied_skill_ids) ? mention.implied_skill_ids : [];
            const ids = uniquePreserveOrder(
                [mention.skill_id, ...implied].filter(
                    (id): id is string => typeof id === 'string' && id !== '',
                ),
            );
            const strength: Strength =
                mention.requirement === 'preferred' ? 'preferred' : mention.requirement === 'bonus' ? 'bonus' : 'required';
            for (const id of ids) record(id, strength);
        }
    }
    ...
    for (const [id, strength] of strongest) {
        if (strength === 'bonus') bonus.add(id);
        else if (strength === 'preferred') preferred.add(id);
...
const jobTimestamp = (job: Job): number | undefined => {
 ...
 */
export const filterJobs = (jobs: readonly Job[], filter: MarketFilter, now?: Date): Job[] => {
    const all: readonly Job[] = Array.isArray(jobs) ? jobs : [];
    const criteria: MarketFilter = filter ?? {};
    ...
    if (isFiniteNumber(recentDays) && recentDays > 0) {
        ...
        if (Number.isFinite(referenceTime)) cutoff = referenceTime - recentDays * MS_PER_DAY;
    }
    ...
    return all.filter((job) => {
        ...
        if (cutoff !== undefined) {
            ...
            if (timestamp !== undefined && timestamp < cutoff) return false;
        }
        ...
        if (categoryFilters.length > 0) {
            ...
            if (category === '' || !categoryFilters.includes(category)) return false;
        }

        if (isFiniteNumber(salaryMinK)) {
            const basis = isFiniteNumber(job.salary_max) ? job.salary_max : job.salary_min;
            if (!isFiniteNumber(basis) || basis < salaryMinK) return false;
        }

        if (isFiniteNumber(experienceMax)) {
            const min = job.experience_min;
            if (isFiniteNumber(min) && min > experienceMax) return false;
        }

        if (educationFilters.length > 0) {
            const education = normalizeText(job.education);
            if (education === '') return false;
            const matched = educationFilters.some(
                (filterEducation) => education === filterEducation || education.includes(filterEducation),
            );
            if (!matched) return false;
        }
 ...
 */
export const computeSkillFrequencies = (
    jobs: readonly Job[],
    analyses: ReadonlyMap<string, JdAnalysis>,
    options: { minCount?: number; skillNames?: ReadonlyMap<string, { name: string; category: string }> } = {},
): SkillFrequency[] => {
    const all: readonly Job[] = Array.isArray(jobs) ? jobs : [];
    ...
    for (const job of all) {
        ...
        for (const skillId of view.present) {
            ...
            if (view.required.has(skillId)) aggregate.required.add(job.job_id);
            if (view.preferred.has(skillId)) aggregate.preferred.add(job.job_id);
            if (view.bonus.has(skillId)) aggregate.bonus.add(job.job_id);
        }
    }
...
const intersectSorted = (a: ReadonlySet<string>, b: ReadonlySet<string>): string[] => {
    const [small, large] = a.size <= b.size ? [a, b] : [b, a];
    const shared: string[] = [];
    for (const id of small) {
        if (large.has(id)) shared.push(id);
    }
    return shared.sort();
};
 ...
 */
export const computeCooccurrence = (
    jobs: readonly Job[],
    analyses: ReadonlyMap<string, JdAnalysis>,
    options: { minCount?: number; topSkills?: number } = {},
): SkillCooccurrence[] => {
    const all: readonly Job[] = Array.isArray(jobs) ? jobs : [];
    const totalJobs = all.length;
    ...
    const requiredJobs = new Map<string, Set<string>>();
    for (const skillId of skillIds) requiredJobs.set(skillId, new Set<string>());
    ...
    for (let i = 0; i < skillIds.length; i += 1) {
        for (let j = i + 1; j < skillIds.length; j += 1) {
            ...
            if (jobIds.length < minCount) continue;
 ...
 */
export const computeCategoryBreakdowns = (
    jobs: readonly Job[],
    analyses: ReadonlyMap<string, JdAnalysis>,
    options: {
    ...
    for (const job of all) {
        ...
        if (bucket === undefined) groups.set(category, [job]);
    ...
    for (const [category, categoryJobs] of groups) {
        ...
        if (minValues.length > 0) breakdown.salary_median_min = roundTo(medianOfSorted(minValues), 1);
        if (maxValues.length > 0) breakdown.salary_median_max = roundTo(medianOfSorted(maxValues), 1);
 ...
 */
export const computeSalaryStats = (jobs: readonly Job[]): SalaryStats => {
    const all: readonly Job[] = Array.isArray(jobs) ? jobs : [];
    const totalJobs = all.length;
...
/** 固定经验桶定义（顺序即输出顺序）。 */
const EXPERIENCE_BUCKETS: readonly { label: string; min: number; max?: number }[] = [
...
/** 按经验**下限**归桶（下界闭合、上界开放）。 */
const bucketByLowerBound = (years: number): string =>
    years < 3 ? '1-3年' : years < 5 ? '3-5年' : years < 10 ? '5-10年' : '10年以上';

/**
 * 仅在「下限为 0 但上限明显大于 1」时改按**上限**归桶，
 * 目的是把 0-3 年这类岗位与真正的应届岗区分开（需求 §七）。
 */
const bucketByUpperBound = (years: number): string =>
    years <= 3 ? '1-3年' : years <= 5 ? '3-5年' : years <= 10 ? '5-10年' : '10年以上';

/** 把一个岗位归入唯一经验桶标签。 */
export const experienceBucketLabel = (job: Job): string => {
    const min = job.experience_min;
    const max = job.experience_max;
    if (!isFiniteNumber(min)) return UNKNOWN_EXPERIENCE_LABEL;
    if (min <= 0) {
        if (!isFiniteNumber(max) || max <= 1) return '应届/1年以内';
 ...
 */
export const computeExperienceDistribution = (jobs: readonly Job[]): ExperienceBucket[] => {
    const all: readonly Job[] = Array.isArray(jobs) ? jobs : [];
    const totalJobs = all.length;

    const counts = new Map<string, number>();
    for (const bucket of EXPERIENCE_BUCKETS) counts.set(bucket.label, 0);
    for (const job of all) {
        if (job === undefined || job === null) continue;
    ...
    return EXPERIENCE_BUCKETS.map((bucket) => {
        ...
        const entry: ExperienceBucket = {
            label: bucket.label,
            min: bucket.min,
            job_count: jobCount,
            job_ratio: ratioOf(jobCount, totalJobs),
        };
        if (bucket.max !== undefined) entry.max = bucket.max;
        return entry;
    });
};
 ...
 */
export const computeCityDistribution = (jobs: readonly Job[]): CityBucket[] => {
    const all: readonly Job[] = Array.isArray(jobs) ? jobs : [];
    const totalJobs = all.length;
    ...
    for (const job of all) {
        ...
        if (bucket === undefined) groups.set(key, [job]);
        else bucket.push(job);
    }
    ...
    for (const [city, cityJobs] of groups) {
        ...
        const entry: CityBucket = {
            city,
            job_count: cityJobs.length,
            job_ratio: ratioOf(cityJobs.length, totalJobs),
        };
        if (minValues.length > 0) entry.salary_median_min = roundTo(medianOfSorted(minValues), 1);
        buckets.push(entry);
    }
...
const normalizeFilter = (filter: MarketFilter | undefined): MarketFilter => {
    const result: MarketFilter = {};
    if (filter === undefined || filter === null) return result;
    if (isFiniteNumber(filter.recent_days) && filter.recent_days > 0) result.recent_days = filter.recent_days;
    if (Array.isArray(filter.cities) && filter.cities.length > 0) result.cities = [...filter.cities];
    if (Array.isArray(filter.categories) && filter.categories.length > 0) {
        result.categories = [...filter.categories];
    }
    if (isFiniteNumber(filter.salary_min_k)) result.salary_min_k = filter.salary_min_k;
    if (isFiniteNumber(filter.experience_max)) result.experience_max = filter.experience_max;
    if (Array.isArray(filter.education) && filter.education.length > 0) result.education = [...filter.education];
    return result;
};

/** 按 `job_id` 去重（保留首次出现），对齐快照「去重后岗位数」的口径。 */
const dedupeJobsById = (jobs: readonly Job[]): Job[] => {
