...
export function apply(ctx, config) {
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    // dsh 0.1.5 no longer exposes the legacy event array during resumed
    // assembly. An absent history means this is the first routed turn.
    const events = Array.isArray(session?.events) ? session.events : []
    if (events.some((event) => event.type === 'tool/call')) {
      return { ...assembled, sections, contexts: [] } // promoted: full catalog
    }

    // The catalog can be populated after an early assembly pass. Leave an
    // incomplete catalog untouched; the next assembly receives the full set.
    const tools = Array.isArray(assembled?.tools) ? assembled.tools : []
    if (tools.length === 0) return { ...assembled, sections, contexts: [] }
    ...
    return {
      ...
      sections,
      contexts: [],
      tools: tools.filter((tool) => core.has(tool.name)),
    }
  })
