...
export function sessionMode(session) {
  // Newer dsh session handles do not expose the legacy in-memory `events`
  // array while a session is being resumed. Treat it as an empty history;
  // the router will use its safe default instead of aborting the agent turn.
