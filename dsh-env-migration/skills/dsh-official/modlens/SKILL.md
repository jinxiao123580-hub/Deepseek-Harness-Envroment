...
| Gemini `User location is not supported` on this machine (China + Clash proxy) | `references/china-proxy-gemini.md`: switch the Clash node to a Gemini-supported egress, verify with the probe, then retry — do NOT touch modlens config |
...
- `Gemini API error 400: User location is not supported`: on this machine the root cause is the Clash node's egress region being rejected by the Gemini API (a working node is 美国 A01 IPv6双栈 Gemini; A09's OVH exit is rejected). See `references/china-proxy-gemini.md`; the fix is switching the node and verifying with the probe — not editing modlens config. `ANTHROPIC_BASE_URL` on this machine points at a text-only deepseek gateway; unset it before any `-p anthropic`/claude-cli vision attempt.
