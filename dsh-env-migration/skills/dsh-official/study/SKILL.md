...
The plugin root is `C:/Users/10905/claude-paper/plugin` (already installed).
Dependencies are already set up on this machine:

- Node >= 18: installed (v24)
- `pdf-parse` npm package: installed in the plugin root (`npm install` done)
- Python 3 + PyMuPDF: use `C:/Users/10905/miniconda3/python.exe` (pymupdf 1.28.2 installed)

When running Python for image extraction, always use:
```bash
C:/Users/10905/miniconda3/python.exe <script> <args>
```

Papers are stored under `D:/claude-papers/` (Windows path; in bash use `D:/claude-papers`).
...
if [[ "$USER_INPUT" =~ ^https?:// ]]; then
  ...
  INPUT_PATH=$(node C:/Users/10905/claude-paper/plugin/skills/study/scripts/download-pdf.cjs "$USER_INPUT")
else
...
* Download PDFs to `C:/Users/10905/AppData/Local/Temp/claude-paper-downloads/`
...
```bash
node C:/Users/10905/claude-paper/plugin/skills/study/scripts/parse-pdf.js "$INPUT_PATH"
```
...
```
D:/claude-papers/papers/{paper-slug}/meta.json
```

Copy original PDF:

```bash
cp <pdf-path> D:/claude-papers/papers/{paper-slug}/paper.pdf
```
...
* `D:/claude-papers/papers/{paper-slug}/meta.json` as `tags`
* `D:/claude-papers/index.json` entry as `tags`
...
```
D:/claude-papers/papers/{paper-slug}/
```
...
```
D:/claude-papers/papers/{paper-slug}/code/
```

Create the code directory first:

```bash
mkdir -p D:/claude-papers/papers/{paper-slug}/code
```
...
**Output path:**
```
D:/claude-papers/papers/{paper-slug}/index.html
```
...
```bash
mkdir -p D:/claude-papers/papers/{paper-slug}/images

C:/Users/10905/miniconda3/python.exe C:/Users/10905/claude-paper/plugin/skills/study/scripts/extract-images.py \
  paper.pdf \
  D:/claude-papers/papers/{paper-slug}/images
```
