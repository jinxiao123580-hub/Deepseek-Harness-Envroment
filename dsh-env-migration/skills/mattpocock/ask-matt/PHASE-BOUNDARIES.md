...
| **Fresh session** | Empty the context window and start from nothing.                  |
...
**2. Is the context irrelevant to what comes next?** Is everything in this session (the exploration, the decisions, the dead ends) disposable? If so, **start a fresh session**. It is the cheapest move on the board: it takes no time and hands back the whole window. Starting fresh also isn't terminal: the old session stays resumable.
